...
def thank_you(request):
    ...
    return render(
        request,
        "usrsmgmnt/thank_you.html",
        {"show_reg_button": False, "sms_panel_is_ok": sms_panel_is_ok},
    )
    ...
    try:
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                logger.info(
                    f"کاربر با نام {user.first_name} {user.last_name}به شماره ملی {user.national_code} و شماره موبایل {user.phone_number}در سایت ثبت نام کرد"
                )
                ...
                if check_sms_panel("option3"):
                    ...
                    logger.info(
                        f"Registration Verification SMS sent to user {form.cleaned_data['first_name'] + ' '+form.cleaned_data['last_name']} and phone number {form.cleaned_data['phone_number']}"
                    )
                else:
                ...
                return render(
                    request,
                    "usrsmgmnt/register.html",
                    {"form": form, "show_reg_button": False},
                )
        ...
        return render(
            request, "usrsmgmnt/register.html", {"form": form, "show_reg_button": False}
        )
    except Exception as e:
        error_message = str(e)
        if "UNIQUE constraint" in str(e):
            messages.error(
                request, "این ایمیل قبلاً ثبت شده است. لطفاً از ایمیل دیگری استفاده کنید."
            )
        ...
        if user is not None:
            ...
            try:
                response = requests.get(
                    fortigate_url, headers=headers, verify=False, timeout=5
                )
                ...
                if response.status_code == 200:
                    ...
                    return JsonResponse(
                        {
                            "success": False,
                            "message": "امکان اتصال به فایروال وجود ندارد",
                        }
                    )

            except requests.Timeout:
                return JsonResponse(
                    {"success": False, "message": "FortiGate connection timed out."}
                )
            ...
            return JsonResponse(
                {"success": False, "message": "نام کاربری یا کلمه عبور اشتباه میباشد"}
            )
    return JsonResponse(
        {"success": False, "message": "متد ارسال درخواست اشتباه میباشد"}
    )
...
def download(request):
    return render(
        request, "usrsmgmnt/download-software.html", {"show_reg_button": False}
    )
...
def run_steps(request):
    try:
            ...
            return JsonResponse(
                {
                    "status": "fail",
                    "step": 1,
                    "error": f"Step 1 failed due to {run_step1_script1()[1]}",
                }
            )
        ...
        if not step2_result:
            return JsonResponse(
                {"status": "fail", "step": 2, "error": "Step 2 failed due to ..."}
            )
        ...
        if not step3_result:
            return JsonResponse(
                {"status": "fail", "step": 3, "error": "Step 3 failed due to ..."}
            )
